          ...
          >
            <div className="flex items-start justify-between p-5 relative">
              <div className="flex-1">
                <h3 className="text-xl font-semibold text-gray-900 mb-3">{order.albumName}</h3>
                ...
                <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-gray-700 text-sm">
                  ...
                  <p><span className="font-medium">Paper:</span> {order.paperType} | <span className="font-medium">Size:</span> {order.albumSize}</p>
                  <p><span className="font-medium">Bag:</span> {order.bagType}</p>
                  <p><span className="font-medium">Delivery:</span> {order.deliveryOption || "N/A"}</p>
                  <p><span className="font-medium">Order Date:</span> {new Date(order.orderDate).toLocaleDateString()}</p>
                  {order.deliveryDate && (
                    <p><span className="font-medium">Delivery Date:</span> {new Date(order.deliveryDate).toLocaleDateString()}</p>
                  )}
                  {order.notes && (
